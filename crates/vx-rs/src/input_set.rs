...
impl WorkspaceFile {
    /// Returns true if this file is from a materialized registry crate
    pub fn is_registry_source(&self) -> bool {
        self.rel_path.starts_with(".vx/registry/")
    }

    /// If this is a registry source, extract the crate name and version
    ///
    /// Returns `Some((name, version))` for paths like `.vx/registry/serde/1.0.197/src/lib.rs`
    pub fn registry_crate_info(&self) -> Option<(&str, &str)> {
        if !self.is_registry_source() {
            return None;
        }

        // Path format: .vx/registry/<name>/<version>/...
        let after_prefix = self.rel_path.strip_prefix(".vx/registry/")?;
...
mod tests {
    ...
    fn test_classify_toolchain_file() {
        ...
        assert!(matches!(classification, PathClassification::Toolchain(_)));
    }
    ...
    #[test]
    fn test_workspace_file_is_registry_source() {
        let registry_file = WorkspaceFile {
            rel_path: ".vx/registry/serde/1.0.197/src/lib.rs".to_string(),
        };
        assert!(registry_file.is_registry_source());

        let regular_file = WorkspaceFile {
            rel_path: "src/lib.rs".to_string(),
        };
        assert!(!regular_file.is_registry_source());

        let nested_file = WorkspaceFile {
            rel_path: "crates/foo/src/main.rs".to_string(),
        };
        assert!(!nested_file.is_registry_source());
    }

    #[test]
    fn test_workspace_file_registry_crate_info() {
        let registry_file = WorkspaceFile {
            rel_path: ".vx/registry/serde/1.0.197/src/lib.rs".to_string(),
        };
        assert_eq!(
            registry_file.registry_crate_info(),
            Some(("serde", "1.0.197"))
        );

        let another_registry = WorkspaceFile {
            rel_path: ".vx/registry/tokio/1.35.0/src/runtime/mod.rs".to_string(),
        };
        assert_eq!(
            another_registry.registry_crate_info(),
            Some(("tokio", "1.35.0"))
        );
        ...
        assert_eq!(regular_file.registry_crate_info(), None);
    }

    #[test]
    fn test_classify_registry_source_as_workspace() {
        // Registry sources under .vx/registry/ should be classified as workspace files
        let workspace_root = Utf8Path::new("/project");
        let path = Utf8Path::new("/project/.vx/registry/serde/1.0.197/src/lib.rs");

        let classification = classify_path(path, workspace_root, &[], &[]);

        match classification {
            PathClassification::Workspace(rel) => {
                assert_eq!(rel, ".vx/registry/serde/1.0.197/src/lib.rs");
                let wf = WorkspaceFile { rel_path: rel };
                assert!(wf.is_registry_source());
                assert_eq!(wf.registry_crate_info(), Some(("serde", "1.0.197")));
            }
            _ => panic!("Expected Workspace classification"),
        }
    }
}
