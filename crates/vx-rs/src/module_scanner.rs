...
//!
//! ## Include macro support (v0.3)
//!
//! The scanner now detects and includes files referenced by:
//! - `include!("path")` - Recursively scanned for further includes/mods
//! - `include_str!("path")` - Added to source closure (no recursion)
//! - `include_bytes!("path")` - Added to source closure (no recursion)
//!
...
//!
//! - **Concatenated paths not supported**: `include!(concat!(env!("OUT_DIR"), "/gen.rs"))`
//!   and similar dynamic path construction is not detected.
//!
//! ## Cache key safety
//!
...
/// A discovered `include!()`, `include_str!()`, or `include_bytes!()` macro invocation
#[derive(Debug, Clone)]
pub struct IncludeMacro {
    /// Path to the included file (relative path from the macro)
    pub path: String,
    /// Line number (1-indexed) for error messages
    pub line: usize,
    /// Macro kind ("include", "include_str", or "include_bytes")
    pub kind: IncludeMacroKind,
}

/// Kind of include macro
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IncludeMacroKind {
    /// `include!("path")`
    Include,
    /// `include_str!("path")`
    IncludeStr,
...
/// Scan a Rust source file for `include!()`, `include_str!()`, and `include_bytes!()` macro invocations.
///
/// Returns all include macro invocations with their relative file paths.
/// Paths are relative to the source file containing the macro.
pub fn scan_include_macros(source: &str) -> Vec<IncludeMacro> {
    let mut out = Vec::new();
    let mut current_line: usize = 1;
    let mut offset: usize = 0;

    // Simple state machine: look for ident! followed by (string_literal)
    let mut last_ident: Option<(IncludeMacroKind, usize)> = None;
    ...
    for token in tokenize(source, FrontmatterAllowed::No) {
        ...
        match token.kind {
            TokenKind::Ident => {
                // Check if this is an include macro name
                let kind = match text {
                    "include" => Some(IncludeMacroKind::Include),
                    "include_str" => Some(IncludeMacroKind::IncludeStr),
                    "include_bytes" => Some(IncludeMacroKind::IncludeBytes),
                    _ => None,
                };

                if let Some(k) = kind {
                    last_ident = Some((k, current_line));
                    seen_bang = false;
                    seen_open_paren = false;
                } else {
                    // Reset if we see a different identifier
                    last_ident = None;
                    seen_bang = false;
                    seen_open_paren = false;
                }
            ...
            TokenKind::Bang => {
                if last_ident.is_some() && !seen_bang {
                    seen_bang = true;
                } else {
                    last_ident = None;
                    seen_bang = false;
            ...
            TokenKind::OpenParen => {
                if last_ident.is_some() && seen_bang && !seen_open_paren {
                    seen_open_paren = true;
                } else if !seen_open_paren {
                    last_ident = None;
                    seen_bang = false;
                }
            }
            TokenKind::Literal { kind, .. } => {
                if let Some((macro_kind, line)) = last_ident {
                    if seen_bang && seen_open_paren {
                        // This should be a string literal
                        if matches!(kind, ra_ap_rustc_lexer::LiteralKind::Str { .. }) {
                            // Extract the string contents (strip quotes)
                            let path = extract_string_literal(text);
                            out.push(IncludeMacro {
                                path,
                                line,
                                kind: macro_kind,
                            });
                ...
                // Reset after seeing literal
                last_ident = None;
                seen_bang = false;
                seen_open_paren = false;
            }
            TokenKind::Whitespace | TokenKind::LineComment { .. } | TokenKind::BlockComment { .. } => {
                // Ignore whitespace and comments, don't reset state
            }
            _ => {
                // Any other token resets our state
                last_ident = None;
                seen_bang = false;
                seen_open_paren = false;
            }
...
/// Extract the contents of a string literal, removing quotes and handling escapes.
///
/// This is a simple version that just strips the outer quotes. For production,
/// you'd want to handle escape sequences properly.
fn extract_string_literal(s: &str) -> String {
    // Handle raw strings: r"...", r#"..."#, etc.
    if s.starts_with("r") {
        let pound_count = s[1..].chars().take_while(|&c| c == '#').count();
        let start = 1 + pound_count + 1; // r + ### + "
        let end = s.len() - 1 - pound_count; // " + ###
        return s[start..end].to_string();
    }

    // Regular string: strip outer quotes
    if s.len() >= 2 {
...
/// by parsing `mod` declarations and `include!()` macro invocations.
/// Also includes files referenced by `include_str!()` and `include_bytes!()`.
/// Returns a sorted, deduplicated list of workspace-relative paths for all
/// source files that make up the crate.
///
/// # Arguments
///
...
pub fn rust_source_closure(
    ...
    while let Some(current_abs) = queue.pop_front() {
        ...
        // Also scan for include!(), include_str!(), include_bytes!() macros
        let includes = scan_include_macros(&source);
        for inc in includes {
            // Resolve path relative to the current source file's directory
            let include_path = source_dir.join(&inc.path);

            // Canonicalize to get absolute path
            let include_path_abs = match include_path.canonicalize_utf8() {
                Ok(p) => p,
                Err(_) => {
                    // File doesn't exist - this might be intentional (cfg-gated include)
                    // or it might be an error. For now, skip it silently.
                    // Future: We could warn or make this configurable.
                    continue;
                }
            };

            // For include!() of Rust source files, recurse into them
            // For include_str!() and include_bytes!(), just add to result (no recursion)
            if inc.kind == IncludeMacroKind::Include {
                // Recurse into included Rust files
                if !visited.contains(&include_path_abs) {
                    queue.push_back(include_path_abs);
                }
            } else {
                // include_str!() and include_bytes!() - just add to result
                if !visited.contains(&include_path_abs) {
                    visited.insert(include_path_abs.clone());
                    let relative = include_path_abs
                        .strip_prefix(&workspace_root)
                        .map(Utf8PathBuf::from)
                        .unwrap_or_else(|_| include_path_abs.clone());
                    result.push(relative);
                }
            }
        }
...
"#;
    ...
    // =========================================================================
    // Tests for include!() macro scanning
    // =========================================================================

    #[test]
    fn test_scan_include_str_macro() {
        let source = r#"
...
"#;
        ...
        assert_eq!(includes[0].path, "README.md");
        assert_eq!(includes[0].kind, IncludeMacroKind::IncludeStr);
    }

    #[test]
    fn test_scan_include_bytes_macro() {
        let source = r#"
...
"#;
        ...
        assert_eq!(includes.len(), 1);
        assert_eq!(includes[0].path, "data.bin");
        assert_eq!(includes[0].kind, IncludeMacroKind::IncludeBytes);
    }

    #[test]
    fn test_scan_include_macro() {
        let source = r#"
...
"#;
        ...
        assert_eq!(includes.len(), 1);
        assert_eq!(includes[0].path, "generated.rs");
        assert_eq!(includes[0].kind, IncludeMacroKind::Include);
    }

    #[test]
    fn test_scan_multiple_includes() {
        let source = r#"
const README: &str = include_str!("README.md");
...
"#;
        ...
        assert_eq!(includes[0].path, "README.md");
        assert_eq!(includes[1].path, "LICENSE");
        assert_eq!(includes[2].path, "data.bin");
        assert_eq!(includes[3].path, "gen.rs");
    }

    #[test]
    fn test_scan_includes_with_paths() {
        let source = r#"
const DATA: &str = include_str!("../../docs/example.md");
"#;
        let includes = scan_include_macros(source);

        assert_eq!(includes.len(), 1);
        assert_eq!(includes[0].path, "../../docs/example.md");
    }

    #[test]
    fn test_source_closure_with_include_str() {
        let (_dir, base) = setup_test_crate(&[
            ("src/lib.rs", r#"const README: &str = include_str!("../README.md");"#),
            ("README.md", "# My Crate"),
        ]);
        ...
        assert_eq!(closure.len(), 2);
        assert!(closure.contains(&Utf8PathBuf::from("src/lib.rs")));
        assert!(closure.contains(&Utf8PathBuf::from("README.md")));
    }

    #[test]
    fn test_source_closure_with_include_bytes() {
        let (_dir, base) = setup_test_crate(&[
            ("src/lib.rs", r#"const DATA: &[u8] = include_bytes!("data.bin");"#),
            ("src/data.bin", "binary data"),
        ]);
        ...
        assert_eq!(closure.len(), 2);
        assert!(closure.contains(&Utf8PathBuf::from("src/lib.rs")));
        assert!(closure.contains(&Utf8PathBuf::from("src/data.bin")));
    }

    #[test]
    fn test_source_closure_with_include_rust() {
        let (_dir, base) = setup_test_crate(&[
            ("src/lib.rs", r#"include!("generated.rs");"#),
        ...
        assert_eq!(closure.len(), 2);
        assert!(closure.contains(&Utf8PathBuf::from("src/lib.rs")));
        assert!(closure.contains(&Utf8PathBuf::from("src/generated.rs")));
    }

    #[test]
    fn test_source_closure_with_nested_includes() {
        // include!() can recursively include other files
        let (_dir, base) = setup_test_crate(&[
            ("src/lib.rs", r#"include!("a.rs");"#),
            ("src/a.rs", r#"include!("b.rs");"#),
            ("src/b.rs", "pub fn leaf() {}"),
        ]);
        ...
        assert_eq!(closure.len(), 3);
        assert!(closure.contains(&Utf8PathBuf::from("src/lib.rs")));
        assert!(closure.contains(&Utf8PathBuf::from("src/a.rs")));
        assert!(closure.contains(&Utf8PathBuf::from("src/b.rs")));
    }

    #[test]
    fn test_extract_string_literal() {
        assert_eq!(extract_string_literal(r#""hello""#), "hello");
        assert_eq!(extract_string_literal(r#""path/to/file.txt""#), "path/to/file.txt");
        assert_eq!(extract_string_literal(r#"r"raw string""#), "raw string");
        // Test raw string with one pound: r#"..."#
        assert_eq!(extract_string_literal("r#\"test\"#"), "test");
        // Test raw string with two pounds: r##"..."##
        assert_eq!(extract_string_literal("r##\"test\"##"), "test");
    }
}
