...
/// Download a tarball and extract to a TreeManifest with hash verification.
///
/// Combines download, verification, and tree extraction.
pub async fn download_and_extract_to_tree(
    url: &str,
    expected_hashes: &[VerifHash],
    compression: Compression,
    strip_components: u32,
    ...
    for attempt in 0..max_retries {
        ...
        match download_and_extract_to_tree_once(
            url,
            expected_hashes,
            compression,
            strip_components,
...
async fn download_and_extract_to_tree_once(
    url: &str,
    expected_hashes: &[VerifHash],
    compression: Compression,
    strip_components: u32,
    ...
    tracing::info!(url = %url, "downloading and extracting to tree");
    ...
    // Extract to tree from verified bytes
    let cursor = std::io::Cursor::new(bytes.to_vec());
    extract_to_tree(cursor, compression, strip_components, store_blob).await
}
